...
async function callOpenAIAPI(apiKey: string, prompt: string, systemPrompt: string | undefined, maxTokens: number, temperature: number = 0.7): Promise<{ data: any; tokenUsage: TokenUsage }> {
    ...
    if (!response.ok) {
        ...
        console.error("OpenRouter (OpenAI) API Error:", errorData);
    ...
    try {
        ...
        console.error("Failed to parse JSON from OpenRouter (OpenAI):", jsonString);
        throw new Error("La respuesta de la IA (OpenAI via OpenRouter) no era un JSON válido.");
    }
}

async function callOpenAITextAPI(apiKey: string, prompt: string, systemPrompt?: string): Promise<{ data: string; tokenUsage: TokenUsage }> {
    const messages: {role: "system" | "user", content: string}[] = [];
    if (systemPrompt) {
        messages.push({ role: 'system', content: systemPrompt });
    }
...
export const categorizeText = async (apiKey: string, text: string): Promise<{ data: string; tokenUsage: TokenUsage }> => {
    ...
    const { data: json, tokenUsage } = await callOpenAIAPI(apiKey, prompt, systemPrompt, 128, 0); // Deterministic
...
${content}`;
    const { data: quantitativeData, tokenUsage: quantitativeUsage } = await callOpenAIAPI(apiKey, scoresPrompt, baseSystemPrompt, 512, 0);
...
**Contenido a analizar:**
${content}`;
        const result = await callOpenAIAPI(apiKey, highlightsPrompt, baseSystemPrompt, 1024, 0.7);
...
`;
  ...
  "comparisonAnalysis": { "keyImprovement": "Mejora clave.", "recurringWeakness": "Debilidad recurrente.", "strategicAdvice": "Consejo." }`;
        ...
        const result = await callOpenAIAPI(apiKey, qualitativePrompt, baseSystemPrompt, 2500, 0.7);
        qualitativeData = result.data;
        qualitativeUsage = result.tokenUsage;
    }
    ...
    const result: AnalysisResult = {
        ...
        ...quantitativeData,
        ...qualitativeData,
        toneAnalysis: undefined // OpenAI text models cannot analyze audio tone in this flow
    };
    ...
    if (!result.title || !result.scores || !result.highlights) {
        throw new Error("La estructura de la respuesta de OpenAI es inválida.");
    }
    
    if (analysisMode === 'full' && (!result.feedback || !result.exercises || !result.improvedText)) {
        console.warn("Respuesta incompleta de OpenAI en modo 'full':", result);
    }
...
**Estructura del JSON requerido (todo el texto en español):**
{
  ...
  "scores": {
    "adaptability": "Puntuación de 0-100 (número entero) sobre la adaptación del vendedor.",
    "questioning": "Puntuación de 0-100 (número entero) sobre la calidad de las preguntas.",
    "objectionHandling": "Puntuación de 0-100 (número entero) sobre el manejo de objeciones.",
    "closing": "Puntuación de 0-100 (número entero) sobre la fortaleza del cierre."
  },
  "keyMoments": [
...
`;
    ...
    const { data: parsedJson, tokenUsage } = await callOpenAIAPI(apiKey, prompt, systemPrompt, 2000);
    ...
    if (!result.title || !result.scores || !result.keyMoments || !result.overallFeedback || !result.exercises) {
        throw new Error("La estructura de la respuesta de OpenAI es inválida para el análisis de role-play.");
    }
...
export const generateScript = async (
    ...
    **Formato de Salida:**
    ...
    return callOpenAITextAPI(apiKey, prompt);
};
